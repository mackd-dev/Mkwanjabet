...
type Bet={id:string;betId:string;bookingCode?:string;type:"Single"|"Accumulator";status:"Open"|"Won"|"Lost"|"Void"|"Cashed out";stake:number;odds:number;returnAmount:number;cashOut?:number;date:string;settledAt?:string;selections:{eventId:string;marketId:string;outcomeId:string;match:string,market:string,pick:string,odd:number,state:string}[]};
...
type Wallet={availableBalanceTzs:number;withdrawableTzs:number;bonusBalanceTzs:number;lockedBalanceTzs:number;bonusWageringRequiredTzs:number;bonusWageringProgressTzs:number;bonusLockedWinningsTzs:number;transactions:ApiTransaction[]};
...
const moneyTsh=(n:number)=>`TSH ${new Intl.NumberFormat("en-TZ",{minimumFractionDigits:2,maximumFractionDigits:2}).format(n)}`;
...
export default function AccountHub({initial="wallet"}:{initial?:View}){
 ...
 const [view,setView]=useState<View>(initial); const [betGroup,setBetGroup]=useState<"Open"|"Settled"|"Virtuals">("Open"); const [hideLost,setHideLost]=useState(false); const [balanceHidden,setBalanceHidden]=useState(false); const [cashOutOffers,setCashOutOffers]=useState<Record<string,number|null>>({}); const [cashOutBusy,setCashOutBusy]=useState<string|null>(null); const [method,setMethod]=useState("M-Pesa"); const [amount,setAmount]=useState("10000"); const [depositPhone,setDepositPhone]=useState(""); const [pendingBooking,setPendingBooking]=useState(""); const [pendingStake,setPendingStake]=useState(0); const [expanded,setExpanded]=useState<string|null>(null); const [notice,setNotice]=useState(""); const [realBets,setRealBets]=useState<Bet[]|null>(null); const [betsNotice,setBetsNotice]=useState("Loading your tickets...");
 ...
 const openBets=useMemo(()=>accountBets.filter(b=>b.status==="Open"),[accountBets]);
 const settledBets=useMemo(()=>accountBets.filter(b=>b.status!=="Open"),[accountBets]);
 const settledShown=useMemo(()=>hideLost?settledBets.filter(b=>b.status!=="Lost"):settledBets,[settledBets,hideLost]);
 const shown=betGroup==="Open"?openBets:betGroup==="Settled"?settledShown:[];
 useEffect(()=>{
  if(betGroup!=="Open")return;
  openBets.forEach(b=>{if(!(b.betId in cashOutOffers))void loadCashOutOffer(b)});
 },[betGroup,openBets]);
 ...
 async function loadCashOutOffer(bet:Bet){
 ...
 async function requestCashOut(bet:Bet){
  setCashOutBusy(bet.id);setNotice("");setWalletError("");
  try{
   const result=await authenticatedApiRequest<{ticketCode:string;status:string;payoutTzs:number}>("/betting/my-bets/"+encodeURIComponent(bet.betId)+"/cash-out",{method:"POST"});
   setNotice(`Cashed out ${bet.id} for ${money(result.payoutTzs)}.`);
   setCashOutOffers(prev=>({...prev,[bet.betId]:null}));
   const data=await authenticatedApiRequest<ApiBet[]>("/betting/my-bets");
   setRealBets(data.map(b=>({...b,date:shortDate(b.date),settledAt:b.settledAt?shortDate(b.settledAt):undefined})));
  }catch(error){setWalletError(apiMessage(error))}
 ...
 return <main className="ah-page">
  <header className="ah-top"><Link className="ah-brand" href="/sports"><img src="/brand/icon/mb-mark-color.png" alt="MkwanjaBet"/><span className="sr-only">MkwanjaBet</span></Link><nav><Link href="/sports">Sports</Link><Link href="/live">Live</Link>{user.role==="ADMIN"&&<Link href="/admin">Admin</Link>}</nav><button className="ah-nav-balance" onClick={()=>setBalanceHidden(!balanceHidden)}><i>{balanceHidden?"◌":"◉"}</i><b>{balanceHidden?"••••••":moneyTsh(wallet?.availableBalanceTzs??0)}</b></button><div className="ah-user"><span>{initials}</span><div><b>{user.name}</b><small>{user.status==="ACTIVE"?"Active account":"Signed in"}</small></div></div></header>
  ...
  <div className="ah-layout">
   ...
   <section className="ah-content">
    {view==="wallet"&&<><div className="ah-heading"><div><span>MY ACCOUNT</span><h1>Wallet overview</h1><p>Your balances and account activity in one secure place.</p></div><button onClick={()=>setView("deposit")}>＋ Deposit</button></div><div className="ah-balances"><article className="main"><small>AVAILABLE BALANCE</small><strong>{money(wallet?.availableBalanceTzs??0)}</strong><p>Ready to bet or withdraw</p><div><button onClick={()=>setView("deposit")}>Deposit</button><button onClick={()=>setView("withdraw")}>Withdraw</button></div></article><article><span>◎</span><small>WITHDRAWABLE</small><strong>{money(wallet?.withdrawableTzs??0)}</strong><p>Available after settled bets</p></article><article><span>★</span><small>BONUS BALANCE</small><strong>{money(wallet?.bonusBalanceTzs??0)}</strong><p>Subject to wagering rules</p></article></div>{wallet&&wallet.bonusWageringRequiredTzs>wallet.bonusWageringProgressTzs&&<div className="ah-ticket-note ah-wagering-note"><b>Bonus wagering in progress</b><span>Stake {money(wallet.bonusWageringRequiredTzs-wallet.bonusWageringProgressTzs)} more to unlock withdrawals on winnings from your bonus-funded bets. Progress: {money(wallet.bonusWageringProgressTzs)} / {money(wallet.bonusWageringRequiredTzs)}{wallet.bonusLockedWinningsTzs>0?` · ${money(wallet.bonusLockedWinningsTzs)} in held winnings will unlock once complete.`:""}</span></div>}<div className="ah-panel"><div className="ah-panel-head"><div><h2>Recent transactions</h2><p>Every wallet movement is recorded.</p></div><button>Download statement</button></div><TransactionTable items={transactions}/></div></>}
    {view==="deposit"&&<><PageTitle eyebrow="WALLET" title={pendingStake?"Fund pending bet":"Deposit funds"} text={pendingStake?"After payment confirms, you will return to the sportsbook to review and confirm the bet.":"Choose your mobile-money provider and enter the amount."}/>{pendingBooking&&<div className="ah-ticket-note"><b>Booking {pendingBooking}</b><span>Intended stake: {money(pendingStake)}. Your funds stay in your wallet unless you confirm and the booking is accepted.</span></div>}<div className="ah-form-grid"><div className="ah-panel"><h2>Select payment method</h2><div className="ah-methods">{methods.map(m=><button className={method===m.name?"active":""} onClick={()=>setMethod(m.name)} key={m.name}><b>{m.code}</b><span><strong>{m.name}</strong><small>{m.hint}</small></span><i>{method===m.name?"●":"○"}</i></button>)}</div><label className="ah-field"><span>Mobile number</span><div><b>+255</b><input inputMode="tel" value={depositPhone} onChange={e=>setDepositPhone(e.target.value.replace(/[^0-9+]/g,""))} placeholder="7XX XXX XXX"/></div></label><label className="ah-field"><span>Deposit amount</span><div><b>TZS</b><input value={amount} onChange={e=>setAmount(sanitizeAmountInput(e.target.value))}/></div></label><div className="ah-chips">{[5000,10000,20000,50000].map(x=><button onClick={()=>setAmount(String(x))} key={x}>{new Intl.NumberFormat().format(x)}</button>)}</div><button className="ah-primary" onClick={()=>submitWallet("deposit")} disabled={walletBusy!==null}>{walletBusy==="deposit"?"Submitting...":`Deposit ${money(Number(amount)||0)}`}</button></div><SummaryCard type="deposit" amount={Number(amount)||0} method={method}/></div></>}
    {view==="withdraw"&&<><PageTitle eyebrow="WALLET" title="Withdraw winnings" text="Send withdrawable funds to a verified mobile-money account."/><div className="ah-form-grid"><div className="ah-panel"><h2>Withdrawal details</h2><label className="ah-field"><span>Send to</span><select value={method} onChange={e=>setMethod(e.target.value)}>{methods.map(m=><option key={m.name}>{m.name}</option>)}</select></label><label className="ah-field"><span>Verified mobile number</span><div><b>+255</b><input value={user.phone.replace(/^\+255/,"")} readOnly/></div></label><label className="ah-field"><span>Amount</span><div><b>TZS</b><input value={amount} onChange={e=>setAmount(sanitizeAmountInput(e.target.value))}/></div></label><div className="ah-withdrawable"><span>Withdrawable balance</span><b>{money(wallet?.withdrawableTzs??0)}</b></div><button className="ah-primary" onClick={()=>submitWallet("withdraw")} disabled={walletBusy!==null}>{walletBusy==="withdraw"?"Submitting...":"Request withdrawal"}</button></div><SummaryCard type="withdraw" amount={Number(amount)||0} method={method}/></div></>}
    {view==="bets"&&<><PageTitle eyebrow="BET HISTORY" title="My bets" text="Real wallet-backed tickets load here when your session has an API token."/><div className="ah-ticket-note"><b>{realBets===null?"Loading tickets":realBets.length?"Account tickets":"No tickets yet"}</b><span>{betsNotice}</span></div><div className="ah-bet-tabs">{(["Open","Settled","Virtuals"] as const).map(x=><button className={betGroup===x?"active":""} onClick={()=>setBetGroup(x)} key={x}>{x}<span>{x==="Open"?openBets.length:x==="Settled"?settledBets.length:0}</span></button>)}</div>{betGroup==="Settled"&&<label className="ah-hide-toggle"><input type="checkbox" checked={hideLost} onChange={e=>setHideLost(e.target.checked)}/><i></i><span><b>Hide lost betslips</b><small>Only won bets will be shown</small></span></label>}<div className="ah-bets">{betGroup==="Virtuals"&&<div className="ah-ticket-note"><b>No virtual bets</b><span>Virtual game tickets aren&apos;t tracked yet.</span></div>}{betGroup!=="Virtuals"&&realBets!==null&&!shown.length&&<div className="ah-ticket-note"><b>No tickets found</b><span>{betGroup==="Open"?"Bets you have open right now will appear here.":"Place a bet from the sportsbook and it will appear here."}</span></div>}{shown.map(b=><article key={b.id} className={`status-${b.status.toLowerCase().replace(" ","-")}`}><header><div className="ah-bet-title"><b>{b.type==="Accumulator"?"Multibet":"Single Bet"}</b>{b.type==="Accumulator"&&<span className="ah-bet-count">{b.selections.length}</span>}</div><i className={`ah-bet-status ${b.status.toLowerCase().replace(" ","-")}`}>{b.status}</i></header><div className="ah-bet-meta"><span>{b.date}</span><span>ID: #{b.id}</span>{b.bookingCode&&<span>Booking {b.bookingCode}</span>}</div><div className="ah-bet-summary"><span><small>ODDS</small><b>{b.odds.toFixed(2)}</b></span><span><small>STAKE</small><b>{money(b.stake)}</b></span><span><small>{b.status==="Won"?"PAYOUT":b.status==="Void"?"REFUND":"POTENTIAL WINNINGS"}</small><b>{money(b.returnAmount)}</b></span>{b.status==="Open"&&(cashOutOffers[b.betId]===undefined?<button disabled>Checking cash out...</button>:cashOutOffers[b.betId]===null?<button disabled>Cash out unavailable</button>:<button className="ah-cashout" disabled={cashOutBusy===b.id} onClick={()=>requestCashOut(b)}>{cashOutBusy===b.id?"Cashing out...":"Cash out "+money(cashOutOffers[b.betId]!)}</button>)}<button onClick={()=>setExpanded(expanded===b.id?null:b.id)}>{expanded===b.id?"Hide":"View"} ticket</button></div>{expanded===b.id&&<div className="ah-selections"><div className="ah-ticket-note"><b>{b.status==="Open"?"Open ticket":b.settledAt?"Settled ticket":"Ticket"}</b><span>{b.status==="Open"?"Stake is locked until this ticket is settled or cashed out.":b.settledAt?`Settled ${b.settledAt}.`:"Ticket details recorded."}</span></div>{b.selections.map(s=><div className="ah-selection-row" key={`${s.eventId}-${s.marketId}-${s.outcomeId}`}><span className={`ah-selection-icon ${s.state.toLowerCase()}`}>{s.state==="WON"?"✓":s.state==="LOST"?"✗":s.state==="VOID"?"–":"○"}</span><div className="ah-selection-info"><b>{s.match}</b><small>{s.market} · {s.pick}</small></div><b className={`ah-selection-odds ${s.state.toLowerCase()}`}>{s.odd.toFixed(2)}</b></div>)}</div>}</article>)}</div></>}

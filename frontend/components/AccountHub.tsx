...
type View = "wallet"|"deposit"|"withdraw"|"bets"|"profile"|"kyc"|"limits"|"notifications";
...
export default function AccountHub({initial="wallet"}:{initial?:View}){
 ...
 const [view,setView]=useState<View>(initial); const [betFilter,setBetFilter]=useState("All"); const [method,setMethod]=useState("M-Pesa"); const [amount,setAmount]=useState("10000"); const [pendingBooking,setPendingBooking]=useState(""); const [pendingStake,setPendingStake]=useState(0); const [expanded,setExpanded]=useState<string|null>("MB-884291"); const [notice,setNotice]=useState(""); const [realBets,setRealBets]=useState<Bet[]|null>(null); const [betsNotice,setBetsNotice]=useState("Showing preview tickets until your real wallet bets load.");
 ...
 const nav:(readonly [View,string,string])[]=[["wallet","Wallet","◈"],["bets","My Bets","▣"],["deposit","Deposit","＋"],["withdraw","Withdraw","↗"],["profile","Profile","◎"],["notifications","Notifications","●"],["kyc","Verification","✓"],["limits","Play limits","◷"]];
 ...
 return <main className="ah-page">
  <header className="ah-top"><Link className="ah-brand" href="/sports"><span>M</span>Mkwanja<b>Bet</b></Link><nav><Link href="/sports">Sports</Link><Link href="/live">Live</Link><Link href="/jackpot">Jackpots</Link><Link href="/promotions">Promotions</Link>{user.role==="ADMIN"&&<Link href="/admin">Admin</Link>}</nav><div className="ah-user"><span>{initials}</span><div><b>{user.name}</b><small>{user.status==="ACTIVE"?"Active account":"Signed in"}</small></div></div></header>

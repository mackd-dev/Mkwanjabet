...
export default function AccountHub({initial="wallet"}:{initial?:View}){
 ...
 useEffect(()=>{
  ...
  async function loadAccount(){
   ...
   if(!mounted)return;
   setUser(currentUser);setSessionReady(true);
   try{
 ...
 return <main className="ah-page">
  ...
  <div className="ah-layout">
   ...
   <section className="ah-content">
    ...
    {view==="profile"&&<><PageTitle eyebrow="ACCOUNT" title="Personal details" text="Keep your contact and security information current."/><div className="ah-panel ah-profile-form"><div className="ah-avatar">{initials}</div><div className="ah-form-two"><label className="ah-field"><span>Full name</span><input defaultValue={user.name}/></label><label className="ah-field"><span>Phone number</span><input defaultValue={user.phone} readOnly/></label><label className="ah-field"><span>Email address</span><input defaultValue={user.email??""}/></label><label className="ah-field"><span>Date of birth</span><input type="date" defaultValue="1994-06-14"/></label></div><button className="ah-primary" onClick={()=>submit("Profile update")}>Save changes</button></div><div className="ah-panel ah-security"><h2>Security</h2><div><span><b>Password</b><small>Last changed 28 days ago</small></span><button>Change password</button></div><div><span><b>Active sessions</b><small>1 device currently signed in</small></span><button>Manage sessions</button></div></div></>}

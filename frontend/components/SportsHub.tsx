...
export default function SportsHub({initialTab="prematch"}:{initialTab?:"prematch"|"live"}={}){
  ...
  const openLoginModal=()=>{setLoginPhone("");setLoginPassword("");setLoginError("");setLoginOpen(true)};
  const submitLogin=async(event:FormEvent<HTMLFormElement>)=>{
    event.preventDefault();
    setLoginBusy(true);setLoginError("");
    try{
      const phone=normalizeIdentifier(loginPhone);
      const session=await apiRequest<{user:SessionUser;accessToken:string;refreshToken:string}>("/auth/login",{method:"POST",body:JSON.stringify({identifier:phone,password:loginPassword})});
      saveSession(session);
      ...
      setLoginError(error instanceof ApiError&&error.payload&&typeof error.payload==="object"&&typeof (error.payload as {message?:unknown}).message==="string"?String((error.payload as {message:string}).message):"Imeshindikana kuingia. Hakikisha namba na nenosiri ni sahihi.");
  ...
  return <main className="sports-shell">
    {loginOpen&&<div className="booking-modal-backdrop sports-login-backdrop" role="presentation" onMouseDown={()=>!loginBusy&&setLoginOpen(false)}><section className="booking-modal sports-login-modal" role="dialog" aria-modal="true" aria-labelledby="sports-login-title" onMouseDown={e=>e.stopPropagation()}><header><div className="sports-login-heading"><img className="sports-login-mark" src="/brand/icon/mb-mark-color.png" alt=""/><div><span>KARIBU TENA</span><h2 id="sports-login-title">Ingia kwenye MkwanjaBet</h2></div></div><button aria-label="Close" onClick={()=>setLoginOpen(false)} disabled={loginBusy}>×</button></header><form className="sports-login-form" onSubmit={submitLogin}><label><span>Namba ya simu</span><div className="phone-field"><b>+255</b><input required autoFocus inputMode="tel" value={loginPhone} onChange={e=>setLoginPhone(e.target.value.replace(/[^0-9+]/g,""))} placeholder="7XX XXX XXX"/></div></label><label><span>Nenosiri</span><div className="password-field"><input required minLength={8} type={loginShowPassword?"text":"password"} value={loginPassword} onChange={e=>setLoginPassword(e.target.value)} placeholder="Angalau tarakimu 8"/><button type="button" onClick={()=>setLoginShowPassword(!loginShowPassword)}>{loginShowPassword?"Ficha":"Onyesha"}</button></div></label>{loginError&&<div className="booking-modal-error">{loginError}</div>}<button className="place-bet-btn sports-login-submit" type="submit" disabled={loginBusy}>{loginBusy?"Inaingia...":"Ingia kwenye Akaunti"}</button><p className="sports-login-switch">Huna akaunti? <Link href="/register">Jiunge sasa</Link></p></form></section></div>}
    ...
    <header className="sports-topbar">
      ...
      <div className="sports-actions">{user?<Link className="wallet-preview" href="/dashboard"><small>Balance</small><b>TZS {(wallet?.availableBalanceTzs??0).toLocaleString("en-US")}</b></Link>:<button className="wallet-preview" onClick={openLoginModal}><small>Balance</small><b>TZS 0</b></button>}{sessionLoading?<span className="sports-session-loading">Checking session...</span>:user?<Link className="sports-register" href="/dashboard">My account</Link>:<><button onClick={openLoginModal}>Log in</button><Link className="sports-register" href="/register">Register</Link></>}</div>
    </header>

...
const promoBanners=[
  ...
  {kicker:"FOOTBALL DAILY",title:"More matches",copy:"Fresh fixtures, teams, leagues and odds from the live feed.",tag:"Daily fixtures",metric:"1X2"},
  {kicker:"BOOKING CODE",title:"Save your slip",copy:"Create a code, fund later, and return to the same ticket.",tag:"Instant code",metric:"CODE"},
  {kicker:"QUICK STAKES",title:"Tap and place",copy:"Use quick amounts, validate odds, and confirm from one betslip.",tag:"Fast slip",metric:"+5K"},
  {kicker:"SECURE WALLET",title:"Wallet-backed bets",copy:"Every stake, payout and deposit is recorded in your account.",tag:"Protected",metric:"SAFE"},
  {kicker:"MOBILE MONEY",title:"Deposit here",copy:"Send a push request and stay on the sports page.",tag:"M-Pesa ready",metric:"PAY"},
  {kicker:"MARKET WATCH",title:"Find odds faster",copy:"Search teams, filter today, and pick from aligned markets.",tag:"Clean board",metric:"ODDS"},
  {kicker:"RESPONSIBLE PLAY",title:"Bet with control",copy:"Set your stake, review your payout, and play within limits.",tag:"18+",metric:"CTRL"},
  {kicker:"MKWANJABET",title:"Green ticket energy",copy:"A sportsbook built for Tanzanian bettors and quick decisions.",tag:"Tanzania",metric:"BET"}
];
  ...
  return <main className="sports-shell">
    ...
    <section className="sports-layout">
      ...
      <section className="sports-content">
        <div className="sports-hero api-football-hero"><div><span>API-FOOTBALL LIVE FEED</span><h1>Real fixtures.<br/>Real club visuals.</h1><p>{featuredEvent?`${featuredEvent.league}: ${featuredEvent.home} vs ${featuredEvent.away}`:"Browse current events, build your ticket and track every wallet-backed bet from one account."}</p><div className="hero-actions-mini"><Link href="#events">Explore events</Link><Link href="/responsible-play">Play responsibly</Link></div></div><div className="feed-banner sports-banner-carousel promo-banner"><div className="promo-banner-copy"><span>{activePromo.kicker}</span><b>{activePromo.title}</b><p>{activePromo.copy}</p><em>{activePromo.tag}</em></div><div className="promo-banner-art"><strong>{activePromo.metric}</strong><i/><i/><i/></div><div className="banner-dots">{promoBanners.map((banner,index)=><button key={banner.kicker} className={index===bannerIndex%promoBanners.length?"active":""} aria-label={`Show banner ${index+1}`} onClick={()=>setBannerIndex(index)}/>)}</div></div><div className="hero-jackpot"><small>YOUR ACCOUNT</small><b>{user?`TZS ${(wallet?.availableBalanceTzs??0).toLocaleString("en-US")}`:"Start betting"}</b><span>{user?"Available wallet balance":"Create an account to fund your wallet and place tickets"}</span>{user?<button onClick={()=>openDepositModal(10000)}>Deposit funds →</button>:<Link href="/register">Register now →</Link>}</div></div>

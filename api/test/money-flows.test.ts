...
test("duplicate SonicPesa success webhooks credit a deposit once",async()=>{
 ...
 const tx:any={notification:{create:async()=>({})},walletTransaction:{findUnique:async()=>entry,updateMany:async()=>{if(state.status==="COMPLETED")return{count:0};state.status="COMPLETED";return{count:1}}},wallet:{update:async({data}:any)=>{state.available+=data.availableBalanceTzs.increment;state.withdrawable+=data.withdrawableTzs.increment;state.credits++;return{userId:"user-1"}}}};

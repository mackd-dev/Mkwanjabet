...
function placementDb(balance=1000){
 ...
 const db:any={outcome:{findUnique:async()=>outcome()},stakeLimit:{findMany:async()=>[]},wallet:{findUnique:async()=>({id:"wallet-1",userId:"user-1",availableBalanceTzs:state.available,lockedBalanceTzs:state.locked,version:state.version})}};
...
test("validation rejects stale odds unless explicitly accepted",async()=>{
 const db:any={outcome:{findUnique:async()=>outcome(2.25)},stakeLimit:{findMany:async()=>[]},wallet:{findUnique:async()=>({availableBalanceTzs:5000})}};
 const service=new BettingService(db);
...
test("concurrent placements cannot overspend one wallet",async()=>{
 const {db,state}=placementDb(1000);const service=new BettingService(db);const dto={selections:[selection],stakeTzs:750,acceptOddsChanges:true};
...
test("winning settlement credits wallet exactly once",async()=>{
 const {db,state}=settlementDb();const service=new BettingService(db);
...
test("void settlement refunds the original stake",async()=>{
 const {db,state}=settlementDb();const service=new BettingService(db);await service.settleOutcome("admin","outcome-1",SelectionStatus.VOID,"abandoned");

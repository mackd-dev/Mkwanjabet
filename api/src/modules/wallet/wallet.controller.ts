...
export class WalletController {
 ...
 @Post("withdraw") async withdraw(@CurrentUser() u:{id:string},@Body() d:MoneyRequestDto){ const w=await this.db.wallet.upsert({where:{userId:u.id},update:{},create:{userId:u.id}}); if(w.withdrawableTzs<d.amountTzs) throw new BadRequestException("Insufficient withdrawable balance"); return this.db.walletTransaction.create({data:{walletId:w.id,type:"WITHDRAWAL",status:"PENDING",amountTzs:-d.amountTzs,provider:d.provider,reference:`WDR-${Date.now()}`,description:`Withdrawal request for ${d.phone}`,metadata:{phone:d.phone}}}); }
}

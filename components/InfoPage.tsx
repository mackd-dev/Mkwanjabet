...
const content: Record<PageKind, { eyebrow: string; title: string; intro: string; sections: { title: string; body: string }[] }> = {
  about: { eyebrow: "KUHUSU PRIMEODDS", title: "Uchambuzi wa mpira ulio wazi na wa kisasa.", intro: "PrimeOdds ni jukwaa la taarifa na uchambuzi wa michezo linalowasaidia watumiaji kuelewa mechi kwa urahisi kabla ya kufanya maamuzi yao wenyewe.", sections: [
  ...
  contact: { eyebrow: "WASILIANA NASI", title: "Tuko hapa kukusaidia.", intro: "Una swali kuhusu account, subscription, malipo au pick? Tuma ujumbe na timu yetu ya msaada itakujibu.", sections: [
    { title: "Msaada wa account", body: "Kwa masuala ya kuingia, OTP, kubadilisha namba au usalama wa account, tumia support@primeodds.co.tz." },
  ...
  responsible: { eyebrow: "CHEZA KWA UWAJIBIKAJI", title: "Maarifa kwanza. Udhibiti daima.", intro: "PrimeOdds hutoa uchambuzi na mapendekezo pekee. Hakuna pick, odds au kiwango cha uhakika kinachoweza kuhakikisha matokeo ya mechi.", sections: [
  ...
  terms: { eyebrow: "MASHARTI YA MATUMIZI", title: "Masharti yanayolinda pande zote.", intro: "Kwa kutumia PrimeOdds unakubali kutumia jukwaa kwa njia halali, kuwajibika kwa maamuzi yako na kuheshimu masharti ya subscription.", sections: [
    { title: "Huduma ya taarifa", body: "PrimeOdds si bookmaker na haipokei dau. Tunatoa maudhui ya uchambuzi, takwimu na mapendekezo ya michezo." },
  ...
  disclaimer: { eyebrow: "TAHADHARI", title: "Hakuna matokeo yanayohakikishwa.", intro: "Takwimu za zamani, odds na confidence score hazihakikishi matokeo yajayo. Matokeo ya michezo yanaweza kubadilika kwa sababu nyingi zisizotabirika.", sections: [
    { title: "Maamuzi ni yako", body: "Maudhui ya PrimeOdds hayapaswi kuchukuliwa kama ushauri wa kifedha. Mtumiaji anawajibika kwa maamuzi na matokeo yake." },
...
export default function InfoPage({ kind }: { kind: PageKind }) {
  ...
  return <main className="info-page">
    <header className="info-nav"><Link className="brand" href="/"><span className="brand-mark">P</span><span>Prime<span>Odds</span></span></Link><nav><Link href="/picks">Picks za Leo</Link><Link href="/results">Matokeo</Link><Link href="/premium">Premium</Link><Link className="btn btn-small btn-gold" href="/login">Ingia</Link></nav></header>
    ...
    <section className="info-cta"><h2>Rudi kwenye mchezo.</h2><p>Angalia picks za leo au fungua akaunti yako ya PrimeOdds.</p><div><Link className="btn btn-gold" href="/picks">Picks za Leo →</Link><Link className="btn btn-outline" href="/dashboard">Dashibodi</Link></div></section>
    <footer className="info-footer"><span>© 2026 PrimeOdds</span><div><Link href="/terms">Masharti</Link><Link href="/privacy">Faragha</Link><Link href="/disclaimer">Tahadhari</Link></div></footer>
  </main>;
}
